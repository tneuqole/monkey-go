...
func TestErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{`{"name": "Monkey"}[fn(x) {x}];`, "not hashable: FUNCTION"},
	}
...
func TestArrayLiterals(t *testing.T) {
	...
	if len(a.Elements) != 3 {
		t.Fatalf("array has wrong number of elements, got=%d", len(a.Elements))
	}

	testIntegerObject(t, a.Elements[0], 1)
	testIntegerObject(t, a.Elements[1], 4)
	testIntegerObject(t, a.Elements[2], 6)
}
...
func testBooleanObject(t *testing.T, obj object.Object, expected bool) bool {
	...
	if result.Value != expected {
		t.Errorf("wrong value, got=%t, want=%t", result.Value, expected)
		return false
	}
